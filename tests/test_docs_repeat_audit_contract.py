...
def test_release_smoke_includes_the_six_documentation_routes_and_js_asset() -> None:
    ...
    assert "_discover_required_doc_redirects" in source
...
def test_audited_legacy_redirects_are_complete_and_not_discoverable() -> None:
    ...
    verifier_path = LANDING_ROOT / "tests" / "verify_live_release.py"
    verifier_source = verifier_path.read_text(encoding="utf-8")
    assert "_discover_required_doc_redirects" in verifier_source
    redirects = runpy.run_path(str(verifier_path))["REQUIRED_DOC_REDIRECTS"]
    assert len(redirects) == 74
