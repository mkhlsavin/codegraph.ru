...
## Почему не графовая СУБД

Граф свойств кода (Code Property Graph, CPG) — это граф. Казалось бы, храни его в Neo4j, JanusGraph или ArangoDB. Но у графовых СУБД есть ряд свойств, которые плохо сочетаются с нашей задачей.
...
Большинство запросов к CPG — не «найди соседей вершины» (для этого графовые БД оптимальны), аналитические агрегации:
...
## Схема: 42 таблицы узлов, 25 таблиц рёбер
...
## Запросы: 11 миксинов вместо ORM
...
## SQL/PGQ: графовые запросы поверх реляционных таблиц
...
## Предвычисленные колонки: как избежать LIKE-сканирования
...
## История compatibility.py: как мы убрали 733K вставок
...
| Графовые алгоритмы | PageRank, WCC, SCC, кратчайший путь (shortest path) | встроенные | через JVM API | нет |
...
## MCP и произвольные SQL-запросы
...
Когда MCP-сервер отдаёт языковой модели результат SQL-запроса, LLM получает структурированные факты из графа, а не сырой код для угадывания. Модель не «понимает» код — она читает предвычисленные метрики, графы вызовов, пути заражения. Вместо «прочитай 6 000 строк и найди проблему» — «вот 5 конкретных путей данных, оцени риск каждого». Структурный анализ для обнаружения, LLM для объяснения.

## Планы: свой форк DuckPGQ с нативными графовыми алгоритмами

DuckPGQ сегодня поддерживает PageRank, компоненты слабой связности (WCC), локальный коэффициент кластеризации и кратчайший путь. Этого достаточно для базового анализа, но нам не хватает двух алгоритмов — и мы реализуем их сами в C++ для последующего PR в основной репозиторий DuckPGQ.
...
Для анализа архитектуры циклические зависимости между модулями — один из главных индикаторов архитектурного долга. В нашем самоанализе (см. [статью 3](habr-03-dogfooding.md)) SCC нашёл 4 таких цикла.
...
Сейчас у нас есть резервная реализация на Python (`PGQComponentAnalyzer`), но она работает через вершинные запросы — на графе с 52 тыс. методов это секунды. Нативный C++ на CSR — миллисекунды.

### Betweenness Centrality — алгоритм Брандеса

Центральность по промежуточности (Betweenness Centrality) показывает, какие методы являются мостами между частями кода: через них проходит наибольшее количество кратчайших путей. Это методы-узкие места — если они сломаются, пострадают все, кто от них зависит.

PageRank отвечает на вопрос «какие методы самые вызываемые», а Betweenness — на вопрос «какие методы самые незаменимые». Метод с низким PageRank, но высоким Betweenness — это тихое узкое место: его вызывают немногие, но если его убрать, граф распадётся.
...
### PGQDataFlowTracer: анализ потоков данных на PGQ

Помимо PR в основной репозиторий, мы переводим анализ потоков данных (см. [статью 5](habr-05-taint-analysis.md)) с рекурсивных CTE на SQL/PGQ MATCH-запросы. Текущий `DataFlowTracer` использует `WITH RECURSIVE` по таблице `edges_reaching_def` (8.2 млн рёбер). Новый `PGQDataFlowTracer` создаёт граф свойств для потоков данных и использует MATCH с переменной длиной пути:
...
SQL-запрос к вашему графу кода — за 5 минут.

---

*CodeGraph — собственный парсер + DuckDB + 733 YAML-отображения. Граф свойств кода для 11 языков. Подробнее: [codegraph.ru](https://codegraph.ru)*
